...
**② スタートG-codeに埋め込んで自動化する** — こちらの方が事故が起きない。ただし埋め込む位置に条件があり、**ベッドメッシュ較正やプライミングライン（ホーミング後にノズルからちょっと押し出す動作）など、Z軸が2mm以上動く処理より後ろに置く必要がある**。それより前に置くと、大きなZ移動をトリガーの終了と誤認して、テストが始まる前に打ち切られてしまう。
