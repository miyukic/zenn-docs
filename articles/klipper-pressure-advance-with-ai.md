...
これはCuraがスライス時にgcodeへ埋め込むような命令ではない。**印刷中にFluidd/MainsailのConsole（G-codeを直接打てる画面）から手動で送る**、M104やM106と同じ扱いのコマンドになる。

```
TUNING_TOWER COMMAND=SET_PRESSURE_ADVANCE PARAMETER=ADVANCE START=0 FACTOR=.020
```

送るタイミングは、1層目（ブリムや土台）が終わって2層目に入った直後が目安。Z=0付近で送っても意味のある変化が出ないので、印刷が始まってノズルがある程度上がってから打ち込む。
