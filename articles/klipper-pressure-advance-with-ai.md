...
**① 印刷中に手動で送る** — Fluidd/MainsailのConsole（G-codeを直接打てる画面）から、M104やM106と同じ扱いで打ち込む。今回はこの方式でやった。目視で1〜2層、あるいは10層くらい進んだのを確認してから打てばいいので、タイミングはそこまで厳密じゃない（ただし今回はそれすら忘れるという事故をやった。後述）。

**② スタートG-codeに埋め込んで自動化する** — 手動送信を忘れるリスクはないが、置く場所には条件がある。ホーミングやベッドメッシュ較正の直後は避ける必要があって、そこで大きなZ移動が起きると、テストが始まる前に打ち切られることがある。①のような「進んでから送る」という融通は効かないので、スタートG-code内でその大きなZ移動の処理が終わった直後の行に置く。
