...
これが一番手こずった。**第1レイヤーがベッドから約2mm浮く**症状が出た。
...
厄介なのは、**SAVE_CONFIG に保存されたメッシュ値（`points =`）は正常に見える**こと。保存値は 0.004〜0.203 と小さい。だが実際にアクティブなメッシュ（`BED_MESH_CALIBRATE` 直後のメモリ上の値）は 1.9 だった。保存値だけ見ていると気づけない。
